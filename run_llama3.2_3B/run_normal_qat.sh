...
tune run --nnodes 1 --nproc_per_node "$NUM_GPUS" qat_distributed --config llama3_2/3B_qat_full \
    epochs="$EPOCHS" \
...
CUDA_VISIBLE_DEVICES=1 tune run quantize --config quantization \
    ...
    checkpointer.model_type=LLAMA3_2 \
...
tune run --nnodes 1 --nproc_per_node "$NUM_GPUS" full_finetune_distributed --config llama3_2/3B_full \
    epochs="$EPOCHS" \
    batch_size="$BATCH_SIZE" \
    gradient_accumulation_steps="$GRADIENT_ACCUMULATION_STEPS" \
    dataset._component_=torchtune.datasets.alpaca_cleaned_dataset \
