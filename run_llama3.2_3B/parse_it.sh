...
BASELINE_LORA_EVAL_FILE="$(find "$BASELINE_LORA_LOG_DIR" -name eval.log)"
BASELINE_LORA_EVAL_FLOAT_FILE="$(find "$BASELINE_LORA_LOG_DIR" -name eval_float.log)"
...
QAT_LORA_EVAL_FILE="$(find "$QAT_LORA_LOG_DIR" -name eval.log)"
...
QAT_EVAL_FILE="$(find "$QAT_LOG_DIR" -name eval.log)"
...
FULL_EVAL_FILE="$(find "$FULL_FINETUNE_LOG_DIR" -name eval.log)"
FULL_EVAL_FLOAT_FILE="$(find "$FULL_FINETUNE_LOG_DIR" -name eval_float.log)"
...
QAT_LORA_TOKENS_PER_SECOND_DECREASE="$(python -c "print(($BASELINE_LORA_TOKENS_PER_SECOND - $QAT_LORA_TOKENS_PER_SECOND) / $BASELINE_LORA_TOKENS_PER_SECOND * 100)")"
QAT_LORA_PEAK_MEMORY_ACTIVE_INCREASE="$(python -c "print(($QAT_LORA_PEAK_MEMORY_ACTIVE - $BASELINE_LORA_PEAK_MEMORY_ACTIVE) / $BASELINE_LORA_PEAK_MEMORY_ACTIVE * 100)")"
QAT_LORA_PEAK_MEMORY_ALLOC_INCREASE="$(python -c "print(($QAT_LORA_PEAK_MEMORY_ALLOC - $BASELINE_LORA_PEAK_MEMORY_ALLOC) / $BASELINE_LORA_PEAK_MEMORY_ALLOC * 100)")"
QAT_LORA_PEAK_MEMORY_RESERVED_INCREASE="$(python -c "print(($QAT_LORA_PEAK_MEMORY_RESERVED - $BASELINE_LORA_PEAK_MEMORY_RESERVED) / $BASELINE_LORA_PEAK_MEMORY_RESERVED * 100)")"
QAT_LORA_PERPLEXITY_RECOVERED="$(python -c "print((1 - ($QAT_LORA_PERPLEXITY - $BASELINE_LORA_FLOAT_PERPLEXITY) / ($BASELINE_LORA_PERPLEXITY - $BASELINE_LORA_FLOAT_PERPLEXITY)) * 100)")"
QAT_LORA_ACC_RECOVERED="$(python -c "print((1 - ($QAT_LORA_ACC - $BASELINE_LORA_FLOAT_ACC) / ($BASELINE_LORA_ACC - $BASELINE_LORA_FLOAT_ACC)) * 100)")"

# QAT LoRA vs regular QAT
QAT_TOKENS_PER_SECOND_INCREASE="$(python -c "print(($QAT_LORA_TOKENS_PER_SECOND - $QAT_TOKENS_PER_SECOND) / $QAT_TOKENS_PER_SECOND * 100)")"
QAT_PEAK_MEMORY_ACTIVE_DECREASE="$(python -c "print(($QAT_PEAK_MEMORY_ACTIVE - $QAT_LORA_PEAK_MEMORY_ACTIVE) / $QAT_PEAK_MEMORY_ACTIVE * 100)")"
QAT_PEAK_MEMORY_ALLOC_DECREASE="$(python -c "print(($QAT_PEAK_MEMORY_ALLOC - $QAT_LORA_PEAK_MEMORY_ALLOC) / $QAT_PEAK_MEMORY_ALLOC * 100)")"
QAT_PEAK_MEMORY_RESERVED_DECREASE="$(python -c "print(($QAT_PEAK_MEMORY_RESERVED - $QAT_LORA_PEAK_MEMORY_RESERVED) / $QAT_PEAK_MEMORY_RESERVED * 100)")"
QAT_PERPLEXITY_DECREASE="$(python -c "print($QAT_PERPLEXITY - $QAT_LORA_PERPLEXITY)")"
QAT_ACC_INCREASE="$(python -c "print(($QAT_LORA_ACC - $QAT_ACC) * 100)")"

# LoRA vs full finetune
LORA_TOKENS_PER_SECOND_INCREASE="$(python -c "print(($BASELINE_LORA_TOKENS_PER_SECOND - $FULL_FINETUNE_TOKENS_PER_SECOND) / $FULL_FINETUNE_TOKENS_PER_SECOND * 100)")"
LORA_PEAK_MEMORY_ACTIVE_DECREASE="$(python -c "print(($FULL_FINETUNE_PEAK_MEMORY_ACTIVE - $BASELINE_LORA_PEAK_MEMORY_ACTIVE) / $FULL_FINETUNE_PEAK_MEMORY_ACTIVE * 100)")"
LORA_PEAK_MEMORY_ALLOC_DECREASE="$(python -c "print(($FULL_FINETUNE_PEAK_MEMORY_ALLOC - $BASELINE_LORA_PEAK_MEMORY_ALLOC) / $FULL_FINETUNE_PEAK_MEMORY_ALLOC * 100)")"
LORA_PEAK_MEMORY_RESERVED_DECREASE="$(python -c "print(($FULL_FINETUNE_PEAK_MEMORY_RESERVED - $BASELINE_LORA_PEAK_MEMORY_RESERVED) / $FULL_FINETUNE_PEAK_MEMORY_RESERVED * 100)")"
LORA_PERPLEXITY_DECREASE="$(python -c "print($FULL_PERPLEXITY - $BASELINE_LORA_PERPLEXITY)")"
LORA_ACC_INCREASE="$(python -c "print(($BASELINE_LORA_ACC - $FULL_ACC) * 100)")"
...
echo "  tokens_per_second_per_gpu % decrease: ${QAT_LORA_TOKENS_PER_SECOND_DECREASE}"
echo "  peak_memory_active % increase: ${QAT_LORA_PEAK_MEMORY_ACTIVE_INCREASE}"
echo "  peak_memory_alloc % increase: ${QAT_LORA_PEAK_MEMORY_ALLOC_INCREASE}"
echo "  peak_memory_reserved % increase: ${QAT_LORA_PEAK_MEMORY_RESERVED_INCREASE}"
echo "  % perplexity degredation recovered: ${QAT_LORA_PERPLEXITY_RECOVERED}"
echo "  % acc_norm degredation recovered: ${QAT_LORA_ACC_RECOVERED}"
echo ""

echo "qat_lora stats compared to raw qat:"
echo "  tokens_per_second_per_gpu % increase: ${QAT_TOKENS_PER_SECOND_INCREASE}"
echo "  peak_memory_active % decrease: ${QAT_PEAK_MEMORY_ACTIVE_DECREASE}"
echo "  peak_memory_alloc % decrease: ${QAT_PEAK_MEMORY_ALLOC_DECREASE}"
echo "  peak_memory_reserved % decrease: ${QAT_PEAK_MEMORY_RESERVED_DECREASE}"
echo "  perplexity decrease: ${QAT_PERPLEXITY_DECREASE}"
echo "  acc_norm increase: ${QAT_ACC_INCREASE} %"
echo ""

echo "baseline_lora stats compared to raw full finetune:"
echo "  tokens_per_second_per_gpu % increase: ${LORA_TOKENS_PER_SECOND_INCREASE}"
echo "  peak_memory_active % decrease: ${LORA_PEAK_MEMORY_ACTIVE_DECREASE}"
echo "  peak_memory_alloc % decrease: ${LORA_PEAK_MEMORY_ALLOC_DECREASE}"
echo "  peak_memory_reserved % decrease: ${LORA_PEAK_MEMORY_RESERVED_DECREASE}"
echo "  perplexity decrease: ${LORA_PERPLEXITY_DECREASE}"
echo "  acc_norm increase: ${LORA_ACC_INCREASE} %"
echo ""
