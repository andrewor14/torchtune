...
if [[ "$USE_QAT" == "True" ]] || [[ "$USE_QAT" == "true" ]]; then
    tune run --nnodes 1 --nproc_per_node "$NUM_GPUS" qat_lora_finetune_distributed --config llama3/8B_qat_lora \
    ...
    tune run --nnodes 1 --nproc_per_node "$NUM_GPUS" lora_finetune_distributed --config llama3/8B_lora \
        batch_size="$BATCH_SIZE" \
        epochs="$EPOCHS" \
        model.lora_rank="$LORA_RANK" \
        model.lora_alpha="$LORA_ALPHA" \
