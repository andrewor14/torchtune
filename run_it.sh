...
FAKE_QUANT_AFTER_N_STEPS="${FAKE_QUANT_AFTER_N_STEPS:-0}"
QUANTIZER="${QUANTIZER:-torchtune.training.quantization.Int8DynActInt4WeightQATQuantizer}"
...
# Set train and eval quantizers
if [[ "$ENABLE_QAT" == "true" ]]; then
    TRAIN_QUANTIZER="$QUANTIZER"
    if [[ "$QUANTIZER" == *NVFP4* ]]; then
        GROUP_SIZE="16"
    fi
else
    TRAIN_QUANTIZER="null"
    GROUP_SIZE="null"
fi
EVAL_QUANTIZER="${QUANTIZER/QAT/}"
if [[ "$QUANTIZER" == *NVFP4* ]]; then
    QUANTIZED_CHECKPOINT_FILE="${QUANTIZED_CHECKPOINT_FILE/8da4w/nvfp4}"
fi
if [[ "$QUANTIZER" == *Float8ActivationInt4Weight* ]]; then
    QUANTIZED_CHECKPOINT_FILE="${QUANTIZED_CHECKPOINT_FILE/8da4w/fp8-int4}"
fi
if [[ "$QUANTIZER" == *Float8ActivationFloat8Weight* ]]; then
    QUANTIZED_CHECKPOINT_FILE="${QUANTIZED_CHECKPOINT_FILE/8da4w/fp8-fp8}"
fi
...
if [[ "$SKIP_FINETUNE" != "true" ]]; then
    ...
    if [[ "$DATASET" == "alpaca" ]]; then
        tune run --nnodes 1 --nproc_per_node "$NUM_GPUS" "$RECIPE" --config "$CONFIG" \
            ...
            fake_quant_after_n_steps="$FAKE_QUANT_AFTER_N_STEPS" \
            quantizer._component_="$TRAIN_QUANTIZER" \
    ...
    if [[ -f "${LAST_EPOCH_CHECKPOINT_DIR}/model.safetensors" ]]; then
        mv "${LAST_EPOCH_CHECKPOINT_DIR}/model.safetensors" "${LAST_EPOCH_CHECKPOINT_DIR}/model-00001-of-00001.safetensors"
    fi
fi
...
if [[ "$SKIP_EVAL" != "true" ]]; then
    ...
    CUDA_VISIBLE_DEVICES="$FIRST_GPU" tune run quantize --config quantization \
        ...
        quantizer._component_="$EVAL_QUANTIZER" \
        quantizer.groupsize="$GROUP_SIZE" \
